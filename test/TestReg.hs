...
recLeftNul y = forAllNullable
  $ \x -> forAllMatching y $ \cs -> accepts y cs ==> accepts (x :> y) cs

recRightNul :: Reg AB -> Property
recRightNul x = forAllNullable
  $ \y -> forAllMatching x $ \cs -> accepts x cs ==> accepts (x :> y) cs
...
gm r n = gmn (simpl r)
 where
  ...
  gmn _ = return []
  splitAt k g1 g2 = liftM2 (++) (g1 k) (g2 (n - k))
